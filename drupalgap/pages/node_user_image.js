...
$('#node_user_image').live('pageshow',function(){
	try {
...
function node_user_image_success(node_user_image) {
	
	// Node title.
	$('#node_user_image h1').html(node_user_image.title);
	
	// Show image.
	img = "";
	if (node_user_image.field_image) {
		img_src = drupalgap_settings.site_path + drupalgap_settings.base_path + node_user_image.field_image[0].filepath;
		img = "<img src='" + img_src + "' />"
	}
	$('#node_user_image .content').html(img);
	
	// Set comments and comment button visibility.
	switch (node_user_image.comment) {
